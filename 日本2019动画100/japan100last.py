...
'''
版本0.014(beta)
后台运行 nohup python3 /root/test/janpan1002.py
...
def get_data(name, browser):
    ...
    with open('./'+wenjianjia+'/'+'jabpan100_last.html', 'w', encoding='utf-8') as f:
    ...
    #定位或切割获取到的文本内容
    tv = body.find('section', id='ranking')
    tv1 = tv.find_all('h2')
    ...
    while(i<len(tv1)):
        ...
        i=i+1
    final.append(temp)
    write_data(final, './'+wenjianjia+'/'+'best100_last.csv')
...
def start():
    try:
        ...
        browser = getSource()
        print("定时周期任务，每日北京时间23:10执行")
        get_data("https://vote.animefestival.jp/", browser)
        ...
        if(browser != None):
            ...
            browser.quit()
        # print(str(err))
        with open('./'+wenjianjia+'/'+'japan100_last_log.txt', 'a', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    scheduler.add_job(start, 'cron', day_of_week='0-6', hour=23, minute=10)
    scheduler.start()
